//! Transfer accumulated fees from vaults to protocol and creator ATAs (50/50 per token).
...
pub fn process(_program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let protocol_fee_a = total_fees_a / 2;
    let creator_fee_a = total_fees_a - protocol_fee_a;
    let protocol_fee_b = total_fees_b / 2;
    let creator_fee_b = total_fees_b - protocol_fee_b;

    if protocol_fee_a > 0 {
        invoke_signed(
            &spl_token::instruction::transfer(token_program.key, token_a_vault.key, protocol_ata_a.key, pool.key, &[], protocol_fee_a)?,
    ...
    if creator_fee_a > 0 {
        invoke_signed(
            &spl_token::instruction::transfer(token_program.key, token_a_vault.key, creator_ata_a.key, pool.key, &[], creator_fee_a)?,
    ...
    if protocol_fee_b > 0 {
        invoke_signed(
            &spl_token::instruction::transfer(token_program.key, token_b_vault.key, protocol_ata_b.key, pool.key, &[], protocol_fee_b)?,
    ...
    if creator_fee_b > 0 {
        invoke_signed(
            &spl_token::instruction::transfer(token_program.key, token_b_vault.key, creator_ata_b.key, pool.key, &[], creator_fee_b)?,
