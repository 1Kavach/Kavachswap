//! Swap with 0.3% fee; accumulate protocol/creator fees in pool for collect_fees.
...
pub fn process(
    ...
    // Swap protection: no zero/dust output, enforce slippage
    if amount_out == 0 {
    ...
    // Accumulate fees in pool state (collect_fees instruction transfers later)
    let clock = Clock::from_account_info(clock)?;
    if a_to_b {
        pool_state.total_fees_a = pool_state
            .total_fees_a
        ...
        pool_state.total_fees_b = pool_state
            .total_fees_b
            .checked_add(fee_amount)
            .ok_or(AmmError::MathOverflow)?;
        pool_state.cumulative_volume_b = pool_state
