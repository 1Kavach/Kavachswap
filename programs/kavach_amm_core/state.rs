//! Pool and account state. Same as 8tier; pool PDA = authority for vaults and LP mint.
...
pub struct Pool {
    ...
    pub lp_mint: Pubkey,
    /// Token program for LP mint (SPL or Token-2022; stored at pool creation)
    pub lp_token_program: Pubkey,
    /// Fee tier numerator (per 10000); must be one of: 1, 5, 10, 25, 50, 80, 100, 125, 150, 200, 250, 300, 400
...
impl Pool {
    pub const LEN: usize = 1 + 1 + (32 * 7) + (8 * 4) + (32 * 2) + (8 * 2) + (16 * 2) + 8;
