//! Burn LP; send token_a and token_b from vaults to user. Token-2022: extension-aware unpack, transfer_checked.
...
pub fn process(
    ...
    invoke(
        &spl_token::instruction::burn(&pool_state.lp_token_program, user_lp_token.key, lp_mint.key, user.key, &[], lp_tokens)?,
    ...
    invoke_signed(
        ...
        &[token_a_vault.clone(), user_token_a.clone(), mint_a.clone(), pool.clone(), token_program_a.clone()],
        signers,
    )?;
    ...
    invoke_signed(
        ...
        &[token_b_vault.clone(), user_token_b.clone(), mint_b.clone(), pool.clone(), token_program_b.clone()],
        signers,
    )?;
