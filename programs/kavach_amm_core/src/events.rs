//! Events for indexers. Same as 8tier.
...
pub fn emit_add_liquidity(
    ...
    let ev = AddLiqEvent { pool, user, amount_a, amount_b, lp_tokens, timestamp };
...
pub fn emit_remove_liquidity(
    ...
    let ev = RemoveLiqEvent { pool, user, lp_tokens, amount_a, amount_b, timestamp };
    let data = ev.try_to_vec()?;
